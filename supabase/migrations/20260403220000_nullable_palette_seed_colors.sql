-- Allow tenants to have no custom palette seeds configured yet.
-- Previously every tenant got factory-default seed colors on creation, making the
-- "Paleta propia del tenant" option appear pre-filled with the same shared colors
-- in every new tenant. Now palette_seed_colors is nullable: NULL means the tenant
-- has not yet configured a custom palette.

alter table public.tenants
  alter column palette_seed_colors drop not null;

-- Relax the shape check to allow NULL.
alter table public.tenants
...
-- Clear the factory-default seed colors from tenants that are NOT actively using
-- the custom palette (i.e. they were set automatically, not by the tenant owner).
update public.tenants
set palette_seed_colors = null
...
-- Recreate create_tenant_with_owner without the hardcoded fallback seed colors.
-- New tenants receive palette_id = 'slate' and palette_seed_colors = NULL.
...
begin
  ...
  values (
    ...
    next_palette_seed_colors,   -- NULL by default; tenant configures later
    auth.uid(),
    auth.uid()
  )

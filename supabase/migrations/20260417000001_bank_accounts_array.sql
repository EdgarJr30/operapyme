-- Replace scalar bank/bank_account columns with a JSONB array so tenants can
-- store multiple bank accounts. Each element: { "bank": text, "account": text }

alter table public.tenants
add column if not exists bank_accounts jsonb not null default '[]'::jsonb;

-- Migrate existing data: if bank or bank_account was set, seed the first entry
update public.tenants
set bank_accounts = jsonb_build_array(
  jsonb_build_object('bank', coalesce(bank, ''), 'account', coalesce(bank_account, ''))
)
where bank is not null or bank_account is not null;
...
-- Replace the RPC to use next_bank_accounts jsonb instead of the two text params

drop function if exists public.update_tenant_branding_settings(uuid, text, text, jsonb, text, text, text, text, text, text, text, text, text, text);
...
create or replace function public.update_tenant_branding_settings(
  ...
  next_bank_accounts jsonb default null,
...
begin
  ...
  set
    ...
    bank_accounts = coalesce(next_bank_accounts, public.tenants.bank_accounts),
    logo_path = case
...
grant execute on function public.update_tenant_branding_settings(uuid, text, text, jsonb, text, text, text, text, text, text, text, jsonb, text) to authenticated;
